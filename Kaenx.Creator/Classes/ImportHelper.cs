    ...
    public class ImportHelper
    {
        ...
        public void ImportComObjects(XElement xcoms)
        {
            ...
            foreach (XElement xcom in xcoms.Elements())
            {
                ...
                if (com.FlagRead == FlagType.Undefined) com.FlagRead = FlagType.Disabled;
                com.FlagWrite = ParseFlagType(xcom.Attribute("WriteFlag")?.Value);
                if (com.FlagWrite == FlagType.Undefined) com.FlagWrite = FlagType.Disabled;
                com.FlagComm = ParseFlagType(xcom.Attribute("CommunicationFlag")?.Value);
                if (com.FlagComm == FlagType.Undefined) com.FlagComm = FlagType.Disabled;
                com.FlagTrans = ParseFlagType(xcom.Attribute("TransmitFlag")?.Value);
                if (com.FlagTrans == FlagType.Undefined) com.FlagTrans = FlagType.Disabled;
                com.FlagUpdate = ParseFlagType(xcom.Attribute("UpdateFlag")?.Value);
                if (com.FlagUpdate == FlagType.Undefined) com.FlagUpdate = FlagType.Disabled;
                com.FlagOnInit = ParseFlagType(xcom.Attribute("ReadOnInitFlag")?.Value);
                if (com.FlagOnInit == FlagType.Undefined) com.FlagOnInit = FlagType.Disabled;
